...
- נכנסים ל [https://react.dev/](https://react.dev/)
- בכלי המפתחים רואים מה קורה כשעוברים ערך חדש
...
[https://reactrouter.com/start/modes](https://reactrouter.com/start/modes)

עכשיו ניצור תקייה חדשה שנקרא לה ROUTES ובתוכה קובץ עם הנתיבים:
...
שימו לב: יש לנו את הנתיבים של האפליקציה, אבל הם חייבים להיות בתוך קומפוננטה שנקראת BROWSER_ROUTER כדי לממש אותם.
...
אז מה שנעשה, בעצם, זה להשתמש בראוטר, אבל הוא לא יקבע מה קורה בכל העמוד אלא רק בחלק מהעמוד. יהיו חלקים מהעמוד שהוא לא יתעסק איתם.
...
נוריד את ה-NAV BAR מהקומפוננטה של הגלידות.

עכשיו שימו לב שאנחנו מנווטים באפליקציה שלנו על ידי שינוי הכתובת URL בצורה ידנית. זה משהו שהוא מאד לא נוח.

לריאקט ראוטר יש מעין אלמנט שנקרא נב-לינק, נשתמש בו בשביל הלינקים:

```jsx
<NavLink to={"/"} className=" text-xs p-3 bg-amber-400 flex flex-row items-center justify-center h-full ">
    בניית גלידה
</NavLink>
<NavLink to={"/about"} className="text-xs p-3 bg-amber-400 flex flex-row items-center justify-center h-full ">
    אודות
</NavLink>
```
...
```jsx
<NavLink to={"/"} className={({ isActive }) => `text-xs p-3 ${isActive ? 'bg-amber-400' : 'bg-amber-secondary'} flex flex-row items-center justify-center h-full `}>
    בניית גלידה
</NavLink>
<NavLink to={"/about"} className={({ isActive }) => `text-xs p-3 ${isActive ? 'bg-amber-400' : 'bg-amber-secondary'} flex flex-row items-center justify-center h-full `}>
    אודות
</NavLink>
```
...
אתם צריכים לבנות עוד נתיב באפליקציה של הזמנות, ותוסיפו לו עוד מקישור בנב-בר.
...
כאן המשימה שלכם, היא איך ליצור 404 עמוד "לא נמצא" עם ריאקט ראוטר. חפשו על זה באינטרנט. שימו לב: יש מספר גרסאות לריאקט ראוטר, בחלק מהגרסאות ה-API שונה.

## פתרון

יש כמה דרכים לעשות את זה. הדרך הכי מקובלת זה עם כוכבית.
...
[https://reactrouter.com/6.30.0/upgrading/v5#note-on-route-path-patterns](https://reactrouter.com/6.30.0/upgrading/v5#note-on-route-path-patterns)

## נתיבים פרטיים/מוגנים

נניח שאנחנו רוצים שחלק מהנתיבים יהיו זמינים רק למי שמחובר לאפליקציה.
...
## נושאים נוספים
אני לא מכסה כאן את כל הדברים בראוטר. אני רוצה לציין גם עוד דברים שלא נגענו בהם, אבל אלו היסודות של הראוטינג ובהם משתמשים ב-90% מהזמן.

### למדריך:
נושאים נוספים שאפשר להזכיר:
- query params
