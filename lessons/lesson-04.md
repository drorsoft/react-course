...
## תוכן עניינים
- [פתיחה](#פתיחה)
- [סוגי קומפוננטות שונים וההתנהגות שלהם](#סוגי-קומפוננטות-שונים-וההתנהגות-שלהם)
- [ריפקטורינג](#ריפקטורינג)
- [איך ריאקט עובד](#איך-ריאקט-עובד)
- [דום וירטואלי](#דום-וירטואלי)
- [הוקים מתקדמים](#הוקים-מתקדמים)
  - [useEffect](#useeffect)
  - [useMemo](#usememo)
  - [useRef](#refs)
...
עכשיו איך נעשה את זה? נשתמש בכלי ה REFACTOR של ה VS-CODE. אפשר כמובן לעשות קופי פייסט בעצמנו, אבל הכלי הזה יעזור לנו לעשות את כל שינויים באימפורט, נתיבים וכו'.
נסמן את כל הקוד בקובץ ונלחץ על Refactor.

צריך לבחור "move to a new file"
...
כרגע הקומפוננטה שלנו `<IceCreamBuilderPage />` מכילה את הלוגיקה של הבנייה, אבל יש בה גם הרבה מאד JSX שעוסק בתצוגה. כדי לעקוב אחרי העקרון של ההפרדה בין קומפוננטות של שליטה, לכאלו של תצוגה, נרצה להוציא את כל הלוגיקה הזו של התצוגה למקום אחר.
...
מה שמיוחד כאן, הוא בעצם כי ב JS, אבל לא בכל שפות התכנות, פונקציות הן function as first class citizen. בעצם כל פונקציה גם פונקציה עם המילה FUNCTION וגם ARROW פונקשנס, הן בעצם יכולות להיות מוגדות בתור משתנים ויותר מזה, אפשר גם להעביר אותן לפונקציות אחרות.
...
אז נתחיל. נבנה קומפוננטה חדשה `IceCreamBuilderControls` ונעתיק לשם את הדברים. כמו שאנחנו מעבירים את הפרופס של סוג התוספות וכו' נעביר גם את הפונקציות שקובעות אותם.
...
יפה! עשינו את הריפקטור הראשון שלנו. זה חשוב מאד כי למשל UNCLE BOB שהוא אחד המפתחים הכי משפיעים ב-20-30 השנים האחרונות, אומר שהעבודה של מפתח היא 50% לדאוג שהקוד יעבוד, ו-50% לעשות ריפקטורינג ונקיון וכו'. למה? כי חשוב לא רק שהקוד יעבוד, אלא שהוא יהיה קריא, וברור, ושיהיה קל לשנות אותו בעתיד.

ריפקטורינג כולל גם להוריד כל מני הערות בקוד שאנחנו לא רוצים, לשנות מיקום של קבצים, לשנות שמות.

אחד הערכים החשובים בפיתוח הוא נקיון וקריאות הקוד, כי רוב הזמן אנחנו קוראים קוד, הרבה יותר ממה שאנחנו כותבים, בין אם זה להוסיף פיצ'ר, לתקן באג, וכו'. וככל שהקוד גדל וגדל, הוא נהיה יותר ויותר מסובך.
...
זה קצת מוזר לקחת את הקומפוננטה ולקרוא לה כפונקציה, אבל בואו נזכור שכל קומפוננטה היא בעצם פשוט פונקציה של JS בסך הכל.

כל קריאה מחדש של חלק מהעץ קומפוננטות שלנו נקרא **Render cycle**. אבל השם מבלבל, כי לא תמיד יש רינדור (שינוי מה שרואים על המסך). עוד מעט נרחיב על זה.
...
לריאקט יש עותק של הDOM, שנראה בערך כמו שאתם רואים כאן: מה האלמנט, מה הסוג שלו, מה הפרופרטיז שלו, כמו קלאס NAMES וכו'. העותק הזה בצורת אובייקט JS.
...
אז בעצם לפי מה שראינו, פונקציית הרנדור (שלא תמיד משנה את הדום עצמה, אבל היא בעצם פונקציה שיוצרת דום וירטואלי) יכולה לרוץ בין 0 ל אין ספור פעמים. אבל מה אם אנחנו רוצים שיהיו דברים שיקרו לא תמיד כשהיא רצה אלא רק בנסיבות מסויימות?
...
עכשיו ההוק הזה מקבל פונקציה ללא ארגומנטים. בואו נראה מתי היא נקראת:
...
אז זה בעצם לא ממש מועיל לנו נכון? אז יש לנו יכולת להוסיף עוד פרמטר ל USE_EFFECT שזהו בעצם מערך שאומר, מתי אנחנו רוצים לקרוא ל USE EFFECT. במערך הזה נשים רשימה של משתנים. רק כשאחד מהם ישתנה USE_EFFECT תיקרא.
...
שימו לב שרק כש TASTE נקרא הקומפוננטה משתנה.
...
אם מעבירים מערך ריק, זה אומר שהפונקציה תקרא רק בפעם הראשונה.
...
אז קודם כל אני אעשה שלא תמיד הקומפוננטה של הטופינגס מתרנדרת, רק כשמופיעה תוספת:
...
```javascript
useEffect(() => {
  const imageElement = document.getElementById('toppings-image');
  const bounceImage = () => {
...
זה טוב ויפה, הבעיה עם AddEventListener זה שהדפדפן שומר את הפונקציה הזו בזיכרון, עד שמסירים אותו. אז בואו נעשה כאן CLEANUP:
...
מה בעצם הוא עושה? הוא משתמש בטכניקה שנקראת ממואזישן. זה דומה למילה memory באנגלית, והיא נובעת גם מהמילה הזו. הטכניקה הזו אומרת כך:

פונקציה שההרצה שלה יקרה, מבחינת זיכרון או עיבוד, אפשר לקרוא לה פעם אחת, לזכור את התוצאה, ופעם הבאה שקוראים לה אם כל הפרמטרים שלה לא השתנו, במקום להריץ אותה שוב, פשוט להחזיר את התוצאה.
...
לכן אני ממליץ להשתמש בזה במקרה שהיישום מתחיל להיות איטי יחסית, אז שימו useMemo ותראו האם זה משפר.

### Refs

דבר נוסף שיש לנו אפשרות לעשות הוא להשתמש במשהו שנקרא ref.

בעצם כמו שלמדנו פונקציה של ריאקט יכולה לרוץ כל פעם מחדש וכל פעם כל מה שקורה בפונקציה לא נשמר. יש דרך לשמר ערך או הפנייה מסויימת, לכל מני מטרות, בעיקר כמו שנראה לקשר לאלמנטי של HTML.
